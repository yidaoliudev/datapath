...
// New creates a new network namespace and returns a handle to it.
func NewAndSave(path string) (ns NsHandle, err error) {
	dir := filepath.Dir(path)
	syscall.Mkdir(dir, syscall.S_IRWXU|syscall.S_IRGRP|syscall.S_IXGRP|syscall.S_IROTH|syscall.S_IXOTH)

	fd, err := syscall.Open(path, syscall.O_RDONLY|syscall.O_CREAT|syscall.O_EXCL, 0)
	if err != nil {
		return -1, err
	}
	...
	if err = syscall.Mount(fmt.Sprintf("/proc/%d/task/%d/ns/net", os.Getpid(), syscall.Gettid()), path, "none", syscall.MS_BIND, ""); err != nil {
		Delete(path)
		return -1, err
...
// New creates a new network namespace and returns a handle to it.
func Save(path string) (ns NsHandle, err error) {
	dir := filepath.Dir(path)
	syscall.Mkdir(dir, syscall.S_IRWXU|syscall.S_IRGRP|syscall.S_IXGRP|syscall.S_IROTH|syscall.S_IXOTH)
